...
const create = async ({ proveedor_id, usuario_id, items }) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    let total = 0;
    for (const item of items) total += item.cantidad * item.precio_unitario;
    const [compraResult] = await connection.query('INSERT INTO compras (proveedor_id, usuario_id, total) VALUES (?,?,?)', [proveedor_id, usuario_id, total]);
    const compra_id = compraResult.insertId;
    for (const item of items) {
      await connection.query('INSERT INTO compras_detalle (compra_id, producto_id, cantidad, precio_unitario) VALUES (?,?,?,?)', [compra_id, item.producto_id, item.cantidad, item.precio_unitario]);
