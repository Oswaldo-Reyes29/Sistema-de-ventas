...
const create = async (ventaData) => {
  const { usuario_id, cliente_nombre, cliente_documento, items } = ventaData;
  ...
  try {
    ...
    const [ventaResult] = await connection.query(
      'INSERT INTO ventas (usuario_id, cliente_nombre, cliente_documento, total) VALUES (?, ?, ?, ?)',
      [usuario_id, cliente_nombre, cliente_documento, total]
    );
    ...
    for (const item of items) {
      ...
      await connection.query(
        ...
        [venta_id, item.producto_id, item.cantidad, prod[0].precio_venta]
      );
      await connection.query(
...
const findByDateRange = async (fecha_inicio, fecha_fin) => {
  try {
    const [rows] = await pool.query(
